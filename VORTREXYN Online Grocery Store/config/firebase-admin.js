...
//   - admin.firestore()  → read/write any Firestore document (used for user list)
//   - admin.auth()       → verify ID tokens, delete user accounts
//
// This module is required by:
//   - routes/admin.js    → list users, delete user accounts
//   - routes/auth.js     → verify Firebase ID tokens on session creation
//
...
// Only initialise once — Node caches require() but some hot-reload
// scenarios can trigger this file twice.
if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert({
      ...
      // RSA private key for the firebase-adminsdk service account.
...
// Export the initialised admin instance.
// Usage: const admin = require('../config/firebase-admin');
